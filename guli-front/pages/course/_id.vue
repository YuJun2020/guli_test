  ...
  <div id="aCoursesList" class="bg-fa of">
    ...
    <section class="container">
      <section class="path-wrap txtOf hLh30">
        ...
        \
        <a href="#" title class="c-999 fsize14">{{ course.onesubject }}</a>
        \
        <span class="c-333 fsize14">{{ course.twosubject }}</span>
      </section>
      <div>
        ...
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            <section class="c-attr-mt" v-if="course.price == 0 || isBuy">
              <a
                :href="'/video/' + chapterList[0].children[0].videoSourceId"
                target="_blank"
                title="立即观看"
...
export default {
  ...
  methods: {
    getCourseDetail(courseId) {
      index.getCourseDetail(courseId).then((response) => {
        this.course = response.data.data.course;
        this.chapterList = response.data.data.chapterList;
        this.isBuy =  response.data.data.isBuy;
        index.updateCourseViewCount(courseId);
      });
    },
    gotoPage(page) {
      index
        .queryCommentList(page, this.pageSize, this.courseId)
        .then((response) => {
    ...
    addComment(){
      ...
      if (userStr!='undefined'&&userStr) {
        ...
        index.addComment(comment).then(response=>{
          this.$message({
              type: 'success',
              message: "评论成功"
        });
        ...
        return callback(new Error("请登录后再评论"));
      }
    },
    createOrders(){
      index.addOrder(this.course.id).then(response=>{
